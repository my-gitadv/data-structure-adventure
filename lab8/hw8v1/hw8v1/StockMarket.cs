...
public class StockMarket
{
    ...
    public void addInvestor(String name, double budget)
    {
        ...
        {
            Console.WriteLine("ERROR! This market limit number of investors to " + maxInvestor);
        }
    }
    ...
    public void addStock(String name, double price)
    {
        ...
        {
            Console.WriteLine("ERROR! This market limit number of stocks to " + maxStock);
        }
    }
    ...
    public int getInvestorID(String investorName)
    {
        ...
        if (investorID == -1)
        {
            Console.WriteLine("ERROR! Investor is not found");
        }
    ...
    public int getStockID(String stockName)
    {
        ...
        if (stockID == -1)
        {
            Console.WriteLine("ERROR! Stock is not found");
        }
    ...
    public void giveMeSomeShares()
    {
        ...
        for (int inv_id = 0; inv_id < numInvestor; inv_id++)
        {
            ...
            while (investors[inv_id].balance > initialBalance / 2)
            {
                ...
                int stock_id = rnd.Next(0, numStock);
    ...
    {
        Console.Write("\t\t\t Number of shares each investor owns \nInv.ID\t");
        for (int i = 0; i < numInvestor; i++)
        {
            Console.Write("[" + i + "]\t");
        }
        ...
        for (int i = 0; i < numStock; i++)
        {
            ...
            for (int j = 0; j < numInvestor; j++)
            {
                Console.Write(stockOwnership[j, i] + "\t");
            }
            Console.WriteLine();
        }
    }
    ...
    public void submitBuyOrder(String investorName, String stockName, int amount, double price)
    {
        ...
        {
            Console.WriteLine("Error! You are about to spend " + amount * price + " baht but you have only " + investors[investorID].balance + " bath");
            return;
        }
        ...
        {
            Console.WriteLine("Error! Bid price must be greater zero");
        }
    }
    ...
    public void submitSellOrder(String investorName, String stockName, int amount, double price)
    {
        ...
        {
            Console.WriteLine("Error! You cannot sell " + amount + " shares of " + stockName + " because you have only " + stockOwnership[investorID, stockID] + " shares of " + stockName);
            return;
        }
        ...
        {
            Console.WriteLine("Error! Offer price must be greater zero");
        }
    }
    ...
    public void matching(char orderCode, int stockID)
    {
        ...
        if (sellerQueue[stockID] != null && sellerQueue[stockID].top() != null && buyerQueue[stockID] != null && buyerQueue[stockID].top() != null)
        {
            ...
            while (bestBuyer.price >= bestSeller.price)
            {
                ...
                Console.WriteLine("Matched!!! Now " + transferAmount + " shares of Stock " + stocks[stockID].name + " are transferred from '" + investors[bestSeller.investorID].name + "' to '" + investors[bestBuyer.investorID].name + "'");
                ...
                Console.WriteLine("Also, " + (dealPrice * transferAmount) + " baht is transferred from '" + investors[bestBuyer.investorID].name + "' to '" + investors[bestSeller.investorID].name + "'");
    ...
    public void portfolio(String investorName)
    {
        ...
        if (investorID > -1)
        {
            ...
            Console.WriteLine("\n-------------------------------------------");
            Console.WriteLine("     Portfolio of '" + investorName + "', ID = " + investorID);
            Console.WriteLine("Current balance is " + totalPortValue + " baht (Buying power)");
            Console.WriteLine("-------------------------------------------");
            Console.WriteLine("Stock\t Amount(shares)   Position Value(baht)");
            for (int i = 0; i < numStock; i++)
            {
                // stockValue = stockPrice * numStockOwn;
                double stockValue = stocks[i].currentPrice * stockOwnership[investorID, i];
                Console.WriteLine(stocks[i].name + "\t\t" + stockOwnership[investorID, i] + "\t\t" + stockValue);
                totalPortValue += stockValue;
            }
            Console.WriteLine("-------------------------------------------");
            Console.WriteLine("Total portfolio values = " + totalPortValue + " bath");
            Console.WriteLine("-------------------------------------------");
        }
        else
        {
            Console.WriteLine("Error! Investor not found");
        }
    }
    ...
    public void quote(String stockName)
    {
        ...
        if (stockID > -1)
        {
            ...
            Console.WriteLine("Stock '" + stockName + "':\t[Best Bid = " + bidAmount + "x" + bestBidPrice + " baht,\t Best Offer = " + offerAmount + "x" + bestOfferPrice + " baht]");
        }
        else
        {
